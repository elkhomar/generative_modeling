...
anims = [torch.load(path + f'/walk_{i}.pt') for i in range(n)]
...
scene = Scene()
l = 1
skeletons = [Skeleton(f"Skeleton{i}", scene, []) for i in range(l)]
rotations_axisa = [torch.Tensor([0, 3, 0]) for i in range(l)]

for i in range(l):
    # latent anim
    skeletons[i].construct_from_zero_pose(SMPL_names, SMPL_positions, SMPL_hierarchy)
    skeletons[i].batch_size = poses[0].shape[0]
    skeletons[i].set_poses_axis_angle(poses[0])
    skeletons[i].bone_list[0].apply_local_rotations_axisangle(rotations_axisa[i])
    skeletons[i].bone_list[0].set_local_positions(torch.Tensor([2*(i%s) - s, 2*(i//s), 1]).unsqueeze(0) + trans_pelvis[0])
    axes = Axes("Axes", skeletons[i].bone_list[0])
axes = Axes("Axes", scene)

scene.render_batch_animation(animation_fps=fps, rendering_fps=60)
