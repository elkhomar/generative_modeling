...
# Static variables, only initialized on the training set and reused for the other datasets
    ...
    def __init__(self, root_path, seq_len, training=False):
        ...
        self.data = torch.cat([self.velocity, self.angular_velocity, self.height, self.keypoints, self.keypoint_velocities, self.rot6d], dim=1)
    ...
    def initialize_keypoints(self):
        ...
        self.height = self.trans[:, 1].unsqueeze(1)
        ...
        dt = 1/self.fps
        self.angular_velocity = torch.cross(self.projected_orientation, self.projected_orientation.roll(1, 0), dim = 1)[:, 1].unsqueeze(1)/dt
        ...
        # 3D Keypoints and rot6D is evaluated from the global matrices of the bones and centered on the pelvis's transform
        bone_pelvis_centered_matrices = [torch.matmul(torch.linalg.inv(skeleton._bone_list[0].global_matrices),skeleton._bone_list[i].global_matrices) for i in range(24)]
        
        # KP3D from the translation vector of the affine matrix
        bone_pelvis_centered_kp = [bone_pelvis_centered_matrices[i][:, :3, 3].unsqueeze(dim=0) for i in range(24)]
        # Rot6d from right, up vectors of the matrix concatenated one after the other
        bone_pelvis_centered_rot6d = [torch.flatten(torch.transpose(bone_pelvis_centered_matrices[i][:, :3, :2], dim0=1, dim1=2), start_dim = 1).unsqueeze(dim=0) for i in range(24)]
        
        # remove Rot6d of the pelvis, feet and hands rotations that are constant in our dataset
        self.indices_to_remove_rot6d = [0, 4, 8, 18, 23]  # Indices of pelvis, lfoot, rfoot, lhand, rhand.
        mask = torch.ones(len(bone_pelvis_centered_rot6d), dtype=bool)
        mask[self.indices_to_remove_rot6d] = False
        bone_pelvis_centered_rot6d = torch.cat(bone_pelvis_centered_rot6d)[mask]

        # remove KP3D of the pelvis
        self.indices_to_remove_kp3d = [0]  # Index of pelvis
        mask = torch.ones(len(bone_pelvis_centered_kp), dtype=bool)
        mask[self.indices_to_remove_kp3d ] = False
        bone_pelvis_centered_kp = torch.cat(bone_pelvis_centered_kp)[mask]


        # Concatenate the joint features [n_joints, n_frames, n_features] to [n_frames, n_features*joints]
        self.keypoints = torch.flatten(torch.transpose(bone_pelvis_centered_kp, 0, 1), start_dim=1)
        self.rot6d = torch.flatten(torch.transpose(bone_pelvis_centered_rot6d, 0, 1), start_dim=1)
    ...
    def model_output_to_pose(self, y):
        ...
        # Complete the rot6d with the removed rotations
        rot6d = torch.cat([rot6d[:, :self.indices_to_remove_rot6d[1]], self.rot6d[:, 1:2], rot6d[:, self.indices_to_remove_rot6d[1]:]], dim=1)
