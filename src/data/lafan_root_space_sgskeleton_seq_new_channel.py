...
# Static variables, only initialized on the training set and reused for the other datasets
    ...
    def __init__(self, root_path, seq_len, training=False):
        ...
        self.data = torch.cat([self.root_velocity_projected, self.angular_velocity, self.keypoints, self.keypoint_velocities, self.rot6d], dim=1)
    ...
    def initialize_keypoints(self):
        ...
        def debug_vis():
            ...
            from kinetix_scenegraph.primitives import Axes, Capsule, Sphere

            scene = Scene()
            axes = Axes("Axes", scene)
            axes = Axes("Axes", skeleton.bone_list[0])

            # Root space axis
            root_axes = Axes("Axes", scene)
            root_axes.batch_size = self.axisa.shape[0]
            root_axes.set_local_positions(self.trans)

            root_position = root_axes.get_global_positions()
            root_position[:, 1] *= 0
            root_axes.set_local_positions(root_position)
            root_axes.set_local_rotations_quaternion(quat_twist)
            
            # Velocity vizualiser
            velocity_axis = Sphere("Velocity", root_axes,(1, 0, 1))
            velocity_axis.batch_size = self.axisa.shape[0]
            velocity_axis.set_local_scales(0.1*torch.ones_like(root_velocity))
            velocity_axis.set_local_positions(root_velocity)
            
            # Angular velocity vizualiser
            angular_velocity_axis = Sphere("Angular Velocity", root_axes,(1, 0, 0))
            angular_velocity_axis.batch_size = self.axisa.shape[0]
            angular_velocity_axis.set_local_scales(0.1*torch.ones_like(root_velocity))
            angular_velocity_axis_pos = angular_velocity_axis.get_local_positions()
            angular_velocity_axis_pos[:, 1] = self.angular_velocity[:, 0]
            angular_velocity_axis.set_local_positions(angular_velocity_axis_pos)

            scene.add(skeleton)
            scene.render_batch_animation(animation_fps=self.fps, rendering_fps=60)
        ...
        # Compute the root velocity in root space
        root_pos = self.trans
        root_velocity = (root_pos - root_pos.roll(1, 0))/dt
        # Express root velocity in root space
        root_velocity = ks.utils.rotation_conversions.quaternion_apply(kin.quaternion_invert(quat_twist), root_velocity)
        root_velocity[:, 1] *= 0
        self.root_velocity_projected = root_velocity[:, [0,2]]
        # Set the first velocity of each animation to the next one
        self.root_velocity_projected[self.wrong_velocity_idx] = self.root_velocity_projected[self.wrong_velocity_idx + np.ones_like(self.wrong_velocity_idx)]
        ...
        reconst_translation = skeleton.bone_list[0].get_global_positions()
        delta_trans_2d = self.root_velocity_projected
        delta_trans_3d = torch.cat([delta_trans_2d[:, 0].unsqueeze(1), torch.zeros_like(delta_trans_2d[:, 0].unsqueeze(1)),delta_trans_2d[:, 1].unsqueeze(1)], dim=1)
        delta_trans_3d = ks.utils.rotation_conversions.quaternion_apply(quat_twist, delta_trans_3d)
        integrated_trans3d = torch.cumsum(delta_trans_3d, dim=0)
        reconst_translation[:, [0, 2]] = integrated_trans3d[:, [0, 2]]*dt

        skeleton.bone_list[0].set_global_positions(reconst_translation)

        ### Integrate rotations

        total_orientation_per_frame = torch.cumsum(self.angular_velocity, dim=0)*dt
        initial_twist = kin.matrix_to_euler_angles(self.initial_rotation_matrices[0], "YZX")
        total_orientation_per_frame += initial_twist[0]

        #total_orientation_per_frame = euler[:, 0].unsqueeze(1)
        ...
        y_pelvis_rotation = torch.zeros(self.angular_velocity.shape[0], 3)
        y_pelvis_rotation[:, 1] = total_orientation_per_frame[:, 0]
        twist = kin.axis_angle_to_quaternion(y_pelvis_rotation)

        original_rot = kin.quaternion_multiply(quat_swing, twist)
        skeleton.bone_list[0].set_local_rotations_quaternion(original_rot)
        debug_vis()"""
    ...
    def model_output_to_pose(self, y, initial_matrix):
        ...
        converts :
          ...
          - an initial rotation matrix of the pelvis [3, 3]
        ...
        #### Integration process
        quat_swing = skeleton.bone_list[0].get_global_rotations_quaternion()
        ### Integrate rotations to get back the twist
        dt = 1/self.fps
        total_orientation_per_frame = torch.cumsum(y[:, 2], dim=0)*dt
        initial_twist = kin.matrix_to_euler_angles(initial_matrix, "YZX")
        total_orientation_per_frame += initial_twist[0]

        character_pelvis_rotation = skeleton.bone_list[0].get_global_rotations_quaternion()
        y_pelvis_rotation = torch.zeros(y.shape[0], 3).to("cuda")
        y_pelvis_rotation[:, 1] = total_orientation_per_frame
        quat_twist = kin.axis_angle_to_quaternion(y_pelvis_rotation)
        ...
        original_rot = kin.quaternion_multiply(quat_swing, quat_twist)
        skeleton.bone_list[0].set_local_rotations_quaternion(original_rot)

        trans = skeleton.bone_list[0].get_global_positions()
...
class LAFANDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test and not self.data_check:
            # Training set
            self.data_train = PoseSequenceDataset(self.hparams.data_dir + 'train/', seq_len=self.hparams.seq_len, training=True)
            self.data_check = PoseSequenceDataset(self.hparams.data_dir + 'val/', seq_len=self.hparams.seq_len, training=False)
...
if __name__ == "__main__":
    ...
    _ = LAFANDataModule(dir_path + "/../../data/LAFAN/", 8)
    _.setup()
    first_idx = 0
    last_idx = 1000
    ...
    #initial_pose = torch.eye(3).to("cuda")
    #initial_pose = _.data_train.initial_rotation_matrices[first_idx].to("cuda")
    torch.save(_.data_train.model_output_to_pose(data_to_reconstruct, initial_pose), "Misc/check_reconstruction.pt")