...
class PoseSequenceDataset(Dataset):
    def __init__(self, root_path, seq_len):
        ...
        self.velocity = self.projected_trans.roll(-1, 0) - self.projected_trans

        # Set the last velocity to the previous one
    ...
    def initialize_keypoints(self):
        ...
        skeleton.bone_list[0].set_local_positions(self.trans)
...
class LAFANDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test and not self.data_check:
            # Training set
            self.data_train = PoseSequenceDataset(self.hparams.data_dir + 'LAFAN locomotion/train/', seq_len=self.hparams.seq_len)
            self.data_check = PoseSequenceDataset(self.hparams.data_dir + 'LAFAN locomotion/val/', seq_len=self.hparams.seq_len)
...
if __name__ == "__main__":
    ...
    _.setup()
    torch.save
    torch.save(_.data_train.model_output_to_pose(_.data_train.data[10000:11000]), "Misc/locomotion_1.pt")
