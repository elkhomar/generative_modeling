...
from kinetix_scenegraph.utils.rotation_conversions import axis_angle_to_matrix, matrix_to_axis_angle
import os
...
class PoseSequenceDataset(Dataset):
    def __init__(self, root_path, seq_len):
        ...
        self.data = axis_angle_to_matrix(self.axisa.reshape(-1, 24, 3)).reshape(-1, 24*9)
    ...
    def model_output_to_pose(y):
        ...
        '''
        n_frames = y.shape[0]
        axisa = matrix_to_axis_angle(y[:, :9*24].reshape(n_frames, 24, 3, 3)).reshape(n_frames, 24, 3)
        pose = {"trans_pelvis" : torch.tensor([0, 0, 0]).repeat(n_frames, 1), "poses" : axisa[:, :9*24].reshape(n_frames, 24, 3)}
        return pose
...
class LAFANDataModule(LightningDataModule):
    ...
    @property
    def n_features(self):
        return self.hparams.seq_len * 9 * 24
...
if __name__ == "__main__":
    ...
    _ = LAFANDataModule(dir_path + "/../../data/", 2)
    _.setup()
    torch.save
    torch.save(_.data_train.model_output_to_pose(_.data_train.data[0:2000]), "Misc/check_reconstruction.pt")
