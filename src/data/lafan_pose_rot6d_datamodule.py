...
class PoseSequenceDataset(Dataset):
    def __init__(self, root_path, seq_len):
        ...
        # Retrieve the poses from the files excluding the T-poses
        ...
        self.data = kin.matrix_to_rotation_6d(kin.axis_angle_to_matrix(self.axisa.reshape(-1, 24, 3))).reshape(-1, 24*6)
    ...
    def model_output_to_pose(y):
        ...
        pose = {"trans_pelvis" : torch.tensor([0, 0, 0]).repeat(n_frames, 1), "poses" : axisa[:, :6*24].reshape(n_frames, 24, 3)}
        return pose
...
class LAFANDataModule(LightningDataModule):
    ...
    @property
    def n_features(self):
        return self.hparams.seq_len * 6 * 24
