...
class beta_VAEModule(LightningModule):
    def __init__(self,
        ...
        encoder.set_dims(input_dim = input_dim)

        # Latent vectors
        self.mu = nn.Linear(encoder.output_dim, latent_dim)
        self.log_var = nn.Linear(encoder.output_dim, latent_dim)

        # Decoder
        self.decoder = decoder
        decoder.set_dims(output_dim = input_dim)
    ...
    def encode(self, x):
        mu = self.mu(self.encoder.model(x))
        log_var = self.log_var(self.encoder.model(x))
        return mu, log_var
...
if __name__ == "__main__":
    _ = beta_VAEModule(None, None, None, None, None, None, None)