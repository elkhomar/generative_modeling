...
class MVAEModule(LightningModule):
    ...
    def forward(self, x):
        # X is supposed to be a sequence of 2 poses [prevpose, pose], returns the predicted pose
    ...
    def loss_function(self, ppred, x, mu, log_var):
        ...
        mse = self.mse(ppred, x[:, self.n_features:2*self.n_features])
    ...
    def training_step_resample(self, batch, batch_idx):
        ...
        # Using the mean of the contributions, could try using self.optimizer to call the optimizer in the loop
        c = batch[:, :self.n_features]
        for i in range(0, self.prediction_length-1):
            # ground truth pi+1, x is (pi_hat, pi+1) exept at the begining of the sequence where it is (p0, p1)
            # condition is pi_hat to try predicting pi+1 exept for p0

            # extract ground truth pi+1 from the batch
            x_gt = batch[:, (i+1)*self.n_features:(i+2)*self.n_features]

            # X is (pi_hat, pi+1) exept at the begining of the sequence where it is (pi, pi+1)
    ...
    def configure_optimizers(self):
            ...
            self.optimizer = lightning.pytorch.core.optimizer.LightningOptimizer(self.optimizer)
