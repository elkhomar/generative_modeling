...
def infer(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    z_single = z_single.repeat(1, 1000, 1) * np.sqrt(model.hparams.latent_dim)
    ...
    idx = cfg.reconstruct_index#Index of the base pose (taken from the training set)
    dataset = datamodule.train_dataloader().dataset
    base_pose = dataset[idx][:dataset.n_features]
    orient_idx = dataset.selectable_indices[idx]
    base_pose_initial_orientation = dataset.initial_rotation_matrices[orient_idx].to("cuda")
    #base_pose_initial_orientation = torch.eye(3).to("cuda")
    ground_truth = dataset[idx:idx+1001][:,:dataset.n_features].to("cuda")
    ...
    for i, trajectory in enumerate(zs):
        ...
        poses = poses.to("cuda")
        experts = torch.zeros(1, model.hparams.n_experts).to("cuda")
        for z in trajectory:
            z = z.to("cuda").unsqueeze(0)
            previous_pose = poses[-1:]
            poses = torch.cat([poses, model.decode(z, previous_pose)])
            experts = torch.cat([experts, model.coefficients])

        experts_list += [experts]
        walks += [poses]

    for i, walk in enumerate(walks):
        anim = datamodule.train_dataloader().dataset.model_output_to_pose(y=walk, initial_matrix=base_pose_initial_orientation)
        # Define the output directory
        directory = os.path.dirname(cfg.ckpt_path)
        file_path = os.path.join(directory, f"walk_{i}_{cfg.reconstruct_index}.pt")
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="random_walk_integrate.yaml")
