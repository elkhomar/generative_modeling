...
# Get reconstruction index from command line arguments
reconstruction_index = sys.argv[2].split('=')[1]
...
skeleton_idx = range(1, 2)
#n = 8

# Compute the best square layout
s = int(np.sqrt(len(skeleton_idx))) + 1



anims = [torch.load(path + f'/walk_{i}_{reconstruction_index}.pt') for i in skeleton_idx]
#anims += [torch.load(path + f'/x_anim_test.pt')]
#poses = [anim["poses"].to("cpu") for anim in anims]
#trans_pelvis = [anim["trans_pelvis"].to("cpu") for anim in anims]
...
skeletons = [Skeleton(f"Skeleton{i}", scene, []) for i in skeleton_idx]

for i, sk in enumerate(skeleton_idx):
    # latent anim
    ...
    skeletons[i].bone_list[0].set_local_positions(torch.Tensor([2*(i%s) - s, 0, 2*(i//s)]).unsqueeze(0) + trans_pelvis[i])
    #skeletons[i].bone_list[0].set_local_positions(trans_pelvis[0])
    axes = Axes("Axes", skeletons[i].bone_list[0])
axes = Axes("Axes", scene)

scene.render_batch_animation(animation_fps=1, rendering_fps=1)
