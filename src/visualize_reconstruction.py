...
anims = [[torch.load(path + f'{animation}_anim_{dataset}.pt') for animation in animations] for dataset in datasets]
poses = [[anims[i][j]["poses"][:5000] for j, animation in enumerate(animations)] for i, dataset in enumerate(datasets)]
trans_pelvis = [[anims[i][j]["trans_pelvis"][:5000] for j, animation in enumerate(animations)] for i, dataset in enumerate(datasets)]

NBR_FRAMES = poses[0][0].shape[0]
...
scene = Scene()
# n is the starting frame of the original anim
n = 0

#original_trans = original_skeleton.get_local_positions()
#original_trans[:, :] = original_trans_pelvis[n:]
#original_skeleton.set_local_positions(original_trans)

# Reconstructed anim
skeletons = [Skeleton(f"Skeleton{i}", scene, []) for i in range(4)]
skeletons = [[Skeleton(f"Skeleton_{dataset}_{animation}", scene, []) for animation in animations] for dataset in datasets]

for i, dataset in enumerate(datasets):
    for j, animation in enumerate(animations):
        # latent anim
        skeletons[i][j].construct_from_zero_pose(SMPL_names, SMPL_positions, SMPL_hierarchy)
        skeletons[i][j].batch_size = poses[i][j].shape[0]
        skeletons[i][j].set_poses_axis_angle(poses[i][j])
        skeletons[i][j].set_local_positions(torch.Tensor([2*i, 2*j, 1]).unsqueeze(0))
